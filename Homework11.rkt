...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Homework11) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;; The first three lines of this file were inserted by DrRacket. They record metadata
;; about the language level of this file in a form that our tools can easily process.
...
(define construct-scope
  (lambda (list-of-2-lists env)
    (cond
      ((null? list-of-2-lists) '())
      (else (cons (list (caar list-of-2-lists) (run-parsed-code (cadar list-of-2-lists) env))
               (construct-scope (cdr list-of-2-lists) (extend-env (list (list (caar list-of-2-lists) (run-parsed-code (cadar list-of-2-lists) env))) env)))))))
...
(define env-let-get-var-values
  (lambda (lst env)
    (cond
      ((null? lst)
        '())
      (else (cons (run-parsed-code (cadar lst) env) (env-let-get-var-values (cdr lst) env))))))
...
(define env '(((a 17))(global (age 21) (a 7) (e b) (c 23))))
;(pop-to-global env)
